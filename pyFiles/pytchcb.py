...
def grdsubkb_CCGC(update,context):
    ...
    update.message.reply_text(text='''Tell me,For which Grade and subject you want me to create class ?''', reply_markup=telegram.ReplyKeyboardMarkup(cs.build_menu(context.user_data['tchgrdsubkb']+['Back'])))
...
def ivgrdsub_CCGC(update,context):
    '''
        Function to send error when user enters Invalid day in Teacher_Timetable/Daily_Timetable path
    '''
    update.message.reply_text(text='''Its not a Grade and Subject from the list\nPlease sent me a Grade and Subject from the list''', reply_markup=telegram.ReplyKeyboardMarkup(cs.build_menu(context.user_data['tchgrdsubkb']+['Back'])))
    return CR8CLS_GRD_KEY
...
def Snd_CR8Cls(update,context: telegram.ext.CallbackContext):
    ...
    if query.data != 'CR8CLS:No':
        ...
        if not tcdata[3] in [grade[0] for grade in db.getTeachtt((update.effective_chat.id),tcdata[4])] :
            ...
            query.edit_message_text(text='''You already have a class at that time:\nCreate class at another period''',parse_mode= 'Markdown' )
            return
        if not chkCR8Cls == -1:
            query.edit_message_text(text='''Please wait I am  forwarding Your message about Created Class to students''',parse_mode= 'Markdown' )
...
def Snd_CXLCls(update,context: telegram.ext.CallbackContext):
    ...
    if query.data != 'CXLCLS:No':
        ...
        if not chkCXLCls == -1:
            query.edit_message_text(text='''Please wait I am  forwarding Your message about Cancelled Class to students''',parse_mode= 'Markdown' )
...
def msgstd_MSMC(update,context):
    ...
    if ((update.message.text) in (context.user_data['tchMsgStdGrdLst'] + ['Message All','Back'])) :
        ...
        update.message.reply_text(text="Send me the message that you want me to pass to Students",parse_mode  = 'Markdown',
                                    reply_markup=telegram.ReplyKeyboardMarkup([['Back']]))
        update.message.reply_text(text="You can Send *POLL*s too (You can find *Poll*(📊) \nin PAPERCLIP button(For SmartPhone) and\nKebab (3 dots or ellipsis) Menu (For Computer))",parse_mode  = 'Markdown')
        return MSGSTD_MSG_KEY
    else:
        update.message.reply_text(text='''I know you won't attend this class.\nSo, I won't *DO IT* 😝\nTo be frank I can't do it''',parse_mode= 'Markdown')
        return cs.END
...
def tch_CT_dev(update,context):
    ...
    update.message.reply_text(text="Send me the message that you want me to pass to Developer(s)",parse_mode  = 'Markdown',
                                    reply_markup=telegram.ReplyKeyboardMarkup([['Back']]))
    update.message.reply_text(text="Tip: Send me a message if you want a permanent change in time table",parse_mode  = 'Markdown')
    return CT_MENU_KEY
